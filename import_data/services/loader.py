...
# ── Função responsavel pela carga sincrona dos dados - versão beta 1

def process_and_load_sync(filepath, dataset_key, delimiter, user_name, tipo_arquivo, conexao_id=None):
    """
    Versão síncrona do process_and_load para uso no scheduler_runner.
    ...
    try:
        ...
        if usar_scd2:
            ...
            if coluna_data:
                ...
                data_max = df[coluna_data].max().date()
                cursor.execute(
                    f"DELETE FROM {table_name} WHERE {coluna_data} between ? and ?",
                    (data_min, data_max)
                )
                conn.commit()
            else:
                ...
                cursor.execute(f"""
                    DELETE FROM {table_name}
                    WHERE Ano = {ano} AND Mes BETWEEN {mes_min} AND {mes_max}
                """)
                conn.commit()
        ...
        mensagem = f"{len(df)} linha(s) importada(s) para {table_name}."
        log.info(f"[sync] Importação concluída: {mensagem}")
        return True, mensagem

    except Exception as e:
        mensagem = str(e)
        log.error(f"[sync] Erro no Loader: {mensagem}")
        if log_id:
            try:
                log_conn   = get_db_connection(DB_CONFIG['log_server'])
                log_cursor = log_conn.cursor()
                ...
                log.error(f"[sync] Erro ao registrar log de erro: {log_err}")
